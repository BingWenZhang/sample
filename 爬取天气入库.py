...
conn=pymysql.connect(host='localhost',user='root',passwd='123456',db='my_database_db',port=3306,charset='utf8')
cursor=conn.cursor()
def get_temperature(url):
    ...
    for info in conmid2:
        ...
        for index, tr in enumerate(tr_list):     # enumerate可以返回元素的位置及内容
            ...
            cursor.execute('insert into air(city,weather,wind,max,min) values(%s,%s,%s,%s,%s)',(city_name,weather,wind,max,min))
